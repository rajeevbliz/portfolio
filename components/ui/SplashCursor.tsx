...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function compileShader(type: any, source: any, keywords: any = null) {
  ...
  `
    ...
    function createDoubleFBO(w: number, h: number, internalFormat: any, format: any, type: any, param: any) {
      ...
      return { width: w, height: h, texelSizeX: 1.0 / w, texelSizeY: 1.0 / h, get read() { return fbo1; }, set read(value) { fbo1 = value; }, get write() { return fbo2; }, set write(value) { fbo2 = value; }, swap() { let temp = fbo1; fbo1 = fbo2; fbo2 = temp; } };
    }
    ...
    function render(target: any) {
      if (config.TRANSPARENT) { gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA); gl.enable(gl.BLEND); }
      else { gl.disable(gl.BLEND); }
      let width = target ? target.width : gl.drawingBufferWidth;
      let height = target ? target.height : gl.drawingBufferHeight;
      gl.viewport(0, 0, width, height);
      if (!config.TRANSPARENT) { gl.clearColor(config.BACK_COLOR.r, config.BACK_COLOR.g, config.BACK_COLOR.b, 1); gl.clear(gl.COLOR_BUFFER_BIT); }
    ...
    // Core coordinate tracking logic
    ...
    const handleMouseDown = (e: MouseEvent) => {
        updatePointerDownData(pointers[0], -1, scaleByPixelRatio(e.clientX), scaleByPixelRatio(e.clientY));
    ...
    const handleTouchStart = (e: TouchEvent) => {
        ...
        updatePointerDownData(pointers[0], touch.identifier, scaleByPixelRatio(touch.clientX), scaleByPixelRatio(touch.clientY));
        splatPointer(pointers[0]);
    };

    const handleTouchMove = (e: TouchEvent) => {
        const touch = e.targetTouches[0];
        updatePointerMoveData(pointers[0], scaleByPixelRatio(touch.clientX), scaleByPixelRatio(touch.clientY));
    };
    ...
    initFramebuffers(); updateKeywords(); update();
